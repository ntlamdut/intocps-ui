...
export class MmConfigurationComponent {
    ...
    loadDiagram() {
        ...
        this.config.fmus.forEach(fmu => {
            ...
            var inputs: any = [];
            var outputs: any = [];
            fmu.scalarVariables.forEach(sv => {
                if (sv.causality == CausalityType.Input) {
                    inputs.push(makePort(sv.name, true));
                } else if (sv.causality == CausalityType.Output) {
                    outputs.push(makePort(sv.name, false));
                }
            });
            makeTemplate(fmu.name, "", "lightgray", inputs, outputs);
        });
